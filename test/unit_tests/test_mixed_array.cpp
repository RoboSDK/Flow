...
void verify(auto& arr) {
  ...
  std::stringstream ss;
  std::for_each(std::begin(arr), std::end(arr), flow::detail::make_visitor([&ss](auto& item) {
         ss << item << " ";
  }));
...
{
  constexpr auto arr = flow::detail::make_mixed_array(1, 2, 3.2, "dog", 100UL);
  verify(arr);
}
...
TEST_CASE("Test constructors and assignment operations", "[mixed_array]")
{
  ...
  {
    constexpr auto moved(flow::detail::make_mixed_array(1, 2, 3.2, "dog", 100UL));
    verify(moved);
  }
}
