...
namespace flow {
    ...
    public:
        ...
        auto createServiceManager(flowProperties properties = flowProperties{}) {
            if constexpr(RequestsDependencies<Impl>) {
                ...
                for (const auto &[key, registration] : cmpMgr->getDependencyRegistry()->_registrations) {
                    const auto &props = std::get<std::optional<flowProperties>>(registration);
                    pushEventInternal<DependencyRequestEvent>(cmpMgr->serviceId(), INTERNAL_EVENT_PRIORITY, cmpMgr, std::get<Dependency>(registration), props.has_value() ? &props.value() : std::optional<flowProperties const *>{});
                }
            ...
            for(auto &[key, mgr] : _services) {
                ...
                for (const auto &[regKey, registration] : depRegistry->_registrations) {
                    if(regKey == InterfaceKey{typeNameHash<Interface>(), Interface::version}) {
                        const auto &props = std::get<std::optional<flowProperties>>(registration);
                        DependencyRequestEvent evt{0, mgr->serviceId(), INTERNAL_EVENT_PRIORITY, mgr, std::get<Dependency>(registration), props.has_value() ? &props.value() : std::optional<flowProperties const *>{}};
