...
namespace flow {
    ...
    struct DependencyRequestEvent final : public Event {
        explicit DependencyRequestEvent(uint64_t _id, uint64_t _originatingService, uint64_t _priority, std::shared_ptr<ILifecycleManager> _manager, Dependency _dependency, std::optional<flowProperties const *> _properties) noexcept :
        ...
        const std::optional<flowProperties const *> properties;
    ...
    struct DependencyUndoRequestEvent final : public Event {
        explicit DependencyUndoRequestEvent(uint64_t _id, uint64_t _originatingService, uint64_t _priority, std::shared_ptr<ILifecycleManager> _manager, Dependency _dependency, flowProperties const * _properties) noexcept :
                Event(TYPE, NAME, _id, _originatingService, _priority), manager(std::move(_manager)), dependency(std::move(_dependency)), properties{_properties} {}
        ~DependencyUndoRequestEvent() final = default;

        const std::shared_ptr<ILifecycleManager> manager;
        const Dependency dependency;
        const flowProperties * properties;
