...
#include <flow/DependencyManager.h>
#include <flow/optional_bundles/logging_bundle/Logger.h>
#include <flow/Service.h>
#include <flow/LifecycleManager.h>
#include <flow/optional_bundles/pubsub_bundle/PubsubZmqAdmin.h>
...
public:
    ...
    void handleDependencyRequest(IPubsubPublisherService*, DependencyRequestEvent const * const evt) {
        ...
        if(runtimeService == end(_scopedPublishers)) {
            _scopedPublishers.emplace(key, getManager()->createServiceManager<IPubsubPublisherService, ZmqPubsubPublisherService>(RequiredList<ILogger>, OptionalList<>, flowProperties{makeProperty<std::string>("scope", scope)}));
        }
    }
    ...
    void handleDependencyRequest(IPubsubSubscriberService*, DependencyRequestEvent const * const evt) {
        ...
        if(runtimeService == end(_scopedSubscribers)) {
            _scopedSubscribers.emplace(key, getManager()->createServiceManager<IPubsubSubscriberService, ZmqPubsubPublisherService>(RequiredList<ILogger>, OptionalList<>, flowProperties{makeProperty<std::string>("scope", scope)}));
        }
    }
