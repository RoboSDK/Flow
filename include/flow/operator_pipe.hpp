...
constexpr auto operator|(is_chain auto&& current_chain, is_transformer_routine auto&& routine)
{
  ...
  return chain<open_chain>(current_chain.frequency, concat(forward(current_chain.routines), forward(routine)));
}
...
constexpr auto operator|(is_chain auto&& current_chain, is_subscriber_routine auto&& routine)
{
  ...
  return chain<closed_chain>(current_chain.frequency, concat(forward(current_chain.routines), forward(routine)));
}
...
constexpr auto operator|(is_chain auto&& current_chain, is_chain_function auto&& function)
{
  ...
  if constexpr (is_init<chain_state>()) {
    ...
    return chain<open_chain>(current_chain.frequency, concat(forward(current_chain.routines), std::move(routine)));
  }
    ...
    if constexpr (is_transformer_function<function_t>) {
      return chain<open_chain>(current_chain.frequency, concat(forward(current_chain.routines), forward(function)));
    }
    else {
      auto routine = detail::to_routine(forward(function));
      return chain<closed_chain>(current_chain.frequency, concat(forward(current_chain.routines), std::move(routine)));
    }
  }
}
