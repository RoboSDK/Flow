...
constexpr auto operator|(is_chain auto&& current_chain, is_transformer_routine auto&& routine)
{
  ...
  return detail::make_appended_chain<open_chain>(__forward(current_chain), __forward(routine));
}
...
constexpr auto operator|(is_chain auto&& current_chain, is_subscriber_routine auto&& routine)
{
  ...
  return detail::make_appended_chain<closed_chain>(__forward(current_chain), __forward(routine));
}
...
constexpr auto operator|(is_chain auto&& current_chain, is_chain_function auto&& function)
{
  ...
  auto routine = detail::to_routine(__forward(function));
...
constexpr auto operator|(is_chain auto&& current_chain, is_chain_spinner auto&& spinner)
{
  ...
  auto routine = detail::to_routine(__forward(spinner));
  return detail::make_appended_chain<closed_chain>(__forward(current_chain), std::move(routine));
}
}// namespace flow