...
/**
 * Create a subscribe to be used by a flow::network
 *
...
auto subscribe(std::function<void(argument_t&&)>&& callback, std::string channel_name)
{
...
template<typename argument_t>
auto subscribe(void (*callback)(argument_t&&), std::string channel_name)
{
...
auto subscribe(auto&& lambda, std::string channel_name)
{
  using callback_t = decltype(lambda);
  return subscribe(detail::metaprogramming::to_function(std::forward<callback_t>(lambda)), std::move(channel_name));
}
